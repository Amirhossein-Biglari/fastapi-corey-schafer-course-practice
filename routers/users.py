...
async def upload_profile_picture(
    user_id: int,
    file: UploadFile,  # special type in fastapi for handling file  uploads
    current_user: CurrentUser,
    db: Annotated[AsyncSession, Depends(get_db)],
):
    ...
    try:
        # image processing with pillow is cpu-bound, doing cpu-bound inside async endpoint will block the even loop
        # normally we use a regular sync function for cpu-bound and fastapi runs it in a thread automatically
        # but we need this endpoint to be async so we can wait our database calls
