...
class Post(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        ForeignKey('users.id'),
        nullable=False,
        index=True  # without it database scans every rows to find a match, primary key gets indexed by default, but foreign key does not, so we need to add index to it, slightly slower writes
    )
